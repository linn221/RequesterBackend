...
func (s *EndpointService) Get(ctx context.Context, id int) (*models.Endpoint, error) {
	...
	if err := s.DB.WithContext(ctx).Preload("Program").Preload("Notes").Preload("Attachments").Preload("Taggables.Tag").First(&endpoint, id).Error; err != nil {
		return nil, err
	}
	return &endpoint, nil
}
...
func (s *EndpointService) List(ctx context.Context) ([]*models.Endpoint, error) {
	...
	if err := s.DB.WithContext(ctx).Preload("Program").Preload("Notes").Preload("Attachments").Preload("Taggables.Tag").Find(&endpoints).Error; err != nil {
		return nil, err
	}
